...
window.addEventListener('message',event=>{
 ...
 if(response?.id && response.id!==lastResponse) {
  ...
  if(response.error) {
   ...
   if(response.recovery_sent || response.password_updated) {
    const email=document.querySelector('#email')?.value||'';
    recoverySent=!!response.recovery_sent;
    if(response.password_updated) authMode='login';
    renderAuth();document.querySelector('#email').value=email;
   }
   if(response.message) {
    if(!A.authenticated) {const el=document.querySelector('#auth-error');el.textContent=response.message;el.classList.remove('hidden');}
...
document.addEventListener('submit',e=>{
 ...
 send(A.auth_mode==='supabase'&&authMode==='recover'&&!recoverySent?'recover_request':authMode,{email:form.get('email'),password:form.get('password')||'',recovery:form.get('recovery')||''});
});
